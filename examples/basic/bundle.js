...
function Demo(sources) {
  var intent = function intent(_ref) {
    ...
    return {
      events$: _rx.Observable.merge(dom.select('#MyElement').events('panleft', options), dom.select('#MyElement').events('panright'), dom.select('#MyElement').events('tap'), dom.select('#MyElement').events('press'))
    };
  };
  ...
  var view = function view(state$) {
    return state$.map(function (state) {
      ...
      return (0, _cycleDom.div)([(0, _cycleDom.h3)('Basic cycle-hammer-driver example'), (0, _cycleDom.h4)('Events: panleft panright tap press'), (0, _cycleDom.div)('#MyElement', type + ' gesture detected')]);
    });
  };
