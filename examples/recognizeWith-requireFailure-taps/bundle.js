...
function Demo(sources) {
  var intent = function intent(_ref) {
    ...
    return {
      event$: _rx.Observable.merge(dom.select('#MyElement').events('singletap', options), dom.select('#MyElement').events('doubletap'))
    };
  };

  var model = function model(actions) {
    return actions.event$.map(function (event) {
      return { event: event };
  ...
  var view = function view(state$) {
    ...
    return state$.map(function (state) {
      ...
      types += event.type + ' ';
      return (0, _cycleDom.div)([(0, _cycleDom.h3)('RecognizeWith and requireFailure cycle-hammer-driver taps example'), (0, _cycleDom.h4)('Events: singletap, doubletap'), (0, _cycleDom.div)('#MyElement', '' + types)]);
    });
  };
